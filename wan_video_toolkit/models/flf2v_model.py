...
from ..types import (
    ModelConfig, GenerationParams, VideoPath, ImageInput, 
    DeviceType, ModelType, DeviceStrategy, LoRAConfig,
    ValidationError, GenerationError, GenerationResult
)
...
class WanFLF2VModel:
    ...
    def __init__(
        ...
        self.pipeline_manager: PipelineManager[WanImageToVideoPipeline] = PipelineManager(
            cache_size=pipeline_cache_size
        )
    ...
    def generate(
        ...
        **kwargs: Any
        ...
        try:
            ...
            first_resized, last_resized, target_h, target_w = self.image_processor.prepare_image_pair(
                first_img,
                last_img,
    ...
    def set_lora_weights(self, adapter_weights: Dict[str, float]) -> None:
        ...
        self.lora_manager.apply_adapters(
            self._pipeline, 
            adapter_names, 
            weights
        )
    ...
    def _create_pipeline(self) -> WanImageToVideoPipeline:
        ...
        try:
            # Load components with explicit types
    ...
    def _load_default_loras(self) -> None:
        ...
        for lora_config in self.config.get("default_loras", []):
            try:
                ...
                print(f"Warning: Failed to load default LoRA {lora_config.get('adapter_name')}: {e}")
